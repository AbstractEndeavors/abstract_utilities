...
def get_dir_filter_kwargs(**kwargs):
    canonical_map = create_canonical_map(keys = "directories")
...
def get_file_filter_kwargs(**kwargs):
    ...
    canonical_map = create_canonical_map(canonical_keys)
    return get_safe_kwargs(canonical_map,**kwargs)
...
def get_file_filters(*args, **kwargs):
    """
    Legacy wrapper to return components from AllParams.
    Used by collect_filepaths and similar functions.
    """
    params = define_all_params(*args, **kwargs)
    return (
        params.directories,
        params,                # full AllParams config
        params.allowed,        # callable
